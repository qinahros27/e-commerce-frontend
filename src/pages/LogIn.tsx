...
import { fetchAllUser, login } from '../redux/reducers/userReducer';
...
const LogIn = () => {
    ...
    const Signin = () => {
        if(email != '' && password != '') {
            ...
            let authenticate:Boolean = checkEmailandPw(email, password);
            if(authenticate) {
                navigate("/");    
            }
            else {
    ...
    const checkEmailandPw = (email:string,password:string) => {
        let Finduser :User[] = users.filter(u => u.email === email && u.password === password);
