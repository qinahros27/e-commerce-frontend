...
const EditProducts = () => {
    ...
    const EditProduct = () => {
        if(title != '' && description != '' && price != 0 && category!= 0 && images.length != 0) {
            dispatch(updateAProduct({productData: {title: title, price: price, description: description, categoryId:category, images: images}, productId: productDetail.id as number}));
            setEdited(true);
        }
        else {
    ...
    return (
        <div className="ManageProduct__manage-box_inner">
            ...
            <div className="ManageProduct__manage-box_input">
                <h4>Category: </h4>
                <select id="category" value={category} name="category" onChange={(e) => setCategory(parseInt(e.target.value))}>
                {categories.map((c) => (
